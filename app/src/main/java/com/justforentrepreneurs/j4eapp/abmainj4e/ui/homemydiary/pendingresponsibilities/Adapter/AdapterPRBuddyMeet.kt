...
class AdapterPRBuddyMeet(activity: FragmentActivity?, companyName:MutableList<FollowUp.Datafollowup>,count_available:Int) :
    RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    private lateinit var mBinding: AdapterPsBuddyMeetBinding
    private var mData:MutableList<FollowUp.Datafollowup> = companyName
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        mContext?.let {
            Glide.with(it)
                    .load(mData.get(position).followup_userprofile)
                    .centerCrop()
                    .placeholder(R.drawable.ic_default_user_svg)
                    .into(mBinding.ivProfileImg)
        }
        if (!mData.get(position).followup_id.equals(readUsingSharedPreference(mContext!!,"userid"))){
        ...
        mViewHolder.mBinding.tcLocation.setText(mData.get(position).followup_date)

        if(mData.get(position).followup_description.length>100){
        ...
        if(!(mData.get(position).followup_username=="" || mData.get(position).followup_username==null)) {
            mViewHolder.mBinding.name.setText(mData.get(position).followup_username.capitalize())
        }else{
            mViewHolder.mBinding.name.setText("")
        }

        if(!(mData.get(position).followup_description=="" || mData.get(position).followup_description==null)) {
            mViewHolder.mBinding.tvDescription.setText(mData.get(position).followup_description.capitalize())
        }else{
            mViewHolder.mBinding.tvDescription.setText("")
        }

        val Datesub: String = mData.get(position).followup_date
        var time: String = mData.get(position).followup_time
        ...
        mViewHolder.mBinding.weak.setText(mData.get(position).followup_day)
        ...
        mViewHolder.mBinding.ivProfileImg.setOnClickListener {
            if (count_available1==0){
                if (mData.get(position).is_viewed==0){
                    ...
                    intent.putExtra("id",mData.get(position).followup_userid)
