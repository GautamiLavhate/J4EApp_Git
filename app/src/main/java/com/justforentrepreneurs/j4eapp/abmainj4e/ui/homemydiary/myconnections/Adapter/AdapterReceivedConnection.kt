...
import com.justforentrepreneurs.j4eapp.databinding.AdapterReceivedConnectionBinding
...
class AdapterReceivedConnection(activity: FragmentActivity?, companyName: MutableList<DataReceived>, var OnClicklk:OnClickLiner, fragment:String,count_available: Int) :
    RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    private lateinit var mBinding: AdapterReceivedConnectionBinding
    private var mData: MutableList<DataReceived> = companyName
    var mContext: Context? = activity
    val fragmentst:String=fragment;
    var count_available1: Int = count_available

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        mBinding = DataBindingUtil.inflate(
            LayoutInflater.from(mContext),
            R.layout.adapter_received_connection,
            parent,
            false
        )
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        if(!(mData.get(position).full_name=="" || mData.get(position).full_name==null)) {
            mViewHolder.mBinding.namerv.setText(wordFirstCap(mData.get(position).full_name))
        }else{
            mViewHolder.mBinding.namerv.setText("")
        }
           ...
           // mContext?.startActivity(Intent(mContext, ProfileActivity::class.java))
            if (count_available1==0){
                if (mData.get(position).is_viewed==0){
                    ...
                    intent.putExtra("type","ReceivedConnection"+fragmentst)
    ...
    class ItemViewHolder internal constructor(
        itemView: View?,
        val mBinding: AdapterReceivedConnectionBinding
    ) :
