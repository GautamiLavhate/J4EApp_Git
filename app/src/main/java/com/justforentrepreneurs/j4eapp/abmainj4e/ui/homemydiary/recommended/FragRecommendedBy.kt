...
import com.justforentrepreneurs.j4eapp.databinding.FragRecommendedByBinding
...
import com.justforentrepreneurs.j4eapp.abmainj4e.ui.homemydiary.recommended.adapters.AdapterRecommendedBy
...
class FragRecommendedBy : BaseFragment(),AdapterRecommendedBy.OnClick2 {

    private lateinit var binding: FragRecommendedByBinding
    var count_available:Int=-1
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        binding = DataBindingUtil.inflate(inflater, R.layout.frag_recommended_by, container, false)
        recyclerData()
        initListener()
        return binding.root
    }
    ...
    private fun recyclerData() {
        ...
        if(isInternetAvailable(requireActivity())){
            app_get_list_recommendations_by("","","")
        }
            ...
            override fun afterTextChanged(s: Editable) {
                if (isInternetAvailable(requireActivity())) {
                    if (s.length > 0) {
                        ...
                        app_get_list_recommendations_by(s.toString(),"","")
                    } else if (s.length == 0) {
                        binding.rvRecommended.setAdapter(null)
                        app_get_list_recommendations_by("","","")
                    }
    ...
    private fun app_get_list_recommendations_by(keyword : String,sortby : String,filterby : String){
        ...
        val call = request.app_get_list_recommendations_by(readUsingSharedPreference(requireActivity(), "userid").toString(),keyword,sortby,filterby)
                ...
                count_available=response.body()?.count_available!!
                    if (response.body()?.status == true) {
                        if (response.body()?.data?.size !=0) {
                            ...
                            val mAdapter = AdapterRecommendedBy(activity, response.body()?.data as MutableList<RecommandByTo.RecommandBydata>,this@FragRecommendedBy)
    ...
    private fun filterBottomSheet() {
        ...
        mBsMemberFilterBinding.NameAscending.setOnClickListener {
            dialog.dismiss()
            if(isInternetAvailable(requireActivity())){
                app_get_list_recommendations_by("","1","")
            }
        }
        ...
        mBsMemberFilterBinding.NameDescending.setOnClickListener {
            dialog.dismiss()
            if(isInternetAvailable(requireActivity())){
                app_get_list_recommendations_by("","2","")
            }
        }
        ...
        mBsMemberFilterBinding.DateAscending.setOnClickListener {
            dialog.dismiss()
            if(isInternetAvailable(requireActivity())){
                app_get_list_recommendations_by("","3","")
            }
        }
        mBsMemberFilterBinding.DateDescending.setOnClickListener {
            dialog.dismiss()
            if(isInternetAvailable(requireActivity())){
                app_get_list_recommendations_by("","4","")
            }
        }
        ...
        mBsMemberFilterBinding.CountAscending.setOnClickListener {
            dialog.dismiss()
            if(isInternetAvailable(requireActivity())){
                app_get_list_recommendations_by("","5","")
            }
        }
        ...
        mBsMemberFilterBinding.CountDescending.setOnClickListener {
            dialog.dismiss()
            if(isInternetAvailable(requireActivity())){
                app_get_list_recommendations_by("","6","")
            }
        }
