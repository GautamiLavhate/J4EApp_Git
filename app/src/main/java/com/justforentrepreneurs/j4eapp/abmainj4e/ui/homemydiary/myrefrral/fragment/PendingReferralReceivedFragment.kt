...
import com.justforentrepreneurs.j4eapp.databinding.FragmentPendingReferralReceivedBinding
import retrofit2.Call
import retrofit2.Callback
import retrofit2.Response


class PendingReferralReceivedFragment : Fragment(),AdapterPendingReferral.OnClick2 {
    private lateinit var binding:FragmentPendingReferralReceivedBinding
    ...
    override fun onCreateView(
        ...
        binding = DataBindingUtil.inflate(inflater,R.layout.fragment_pending_referral_received, container, false)
        recyclerData()
        return binding.root
    }
    ...
    private fun getRequirements(){
        ...
        val call = request.app_get_pending_referral_received(readUsingSharedPreference(requireActivity(), "userid").toString())
                    ...
                    if (response.body()?.status == true) {
                        if (response.body()?.data != null) {
                            ...
                            val mAdapter = AdapterPendingReferral(
                                activity,
                                response.body()?.data as MutableList<Data>,this@PendingReferralReceivedFragment,"Receive")
