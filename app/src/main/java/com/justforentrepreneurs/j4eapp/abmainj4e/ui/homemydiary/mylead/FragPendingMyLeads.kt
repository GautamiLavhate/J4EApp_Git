...
import com.justforentrepreneurs.j4eapp.abmainj4e.ui.homemydiary.mylead.adapter.AdapterPendingMyLead
...
class FragPendingMyLeads : BaseFragment(),AdapterPendingMyLead.OnClick2{
    ...
    private fun getMyPendingLead(){
        val request = ServiceBuilder.buildService(APIinterface::class.java)
        val call = request.app_get_pending_leads(readUsingSharedPreference(requireActivity(), "userid").toString())
        call.enqueue(object : Callback<MyLeadCom> {
            override fun onResponse(
                    call: Call<MyLeadCom>,
                    response: Response<MyLeadCom>
            ) {
                dismissLoader();
                if (response.isSuccessful) {
                    if (response.body()?.status == true) {
                        if (response.body()?.user_data != null) {
                            ...
                            val mAdapter = AdapterPendingMyLead(
                                    activity,
                                    response.body()?.user_data as MutableList<MyLeadCom.MyRequirementpending>
                            ,this@FragPendingMyLeads)
            ...
            override fun onFailure(call: Call<MyLeadCom>, t: Throwable) {
                dismissLoader();
            }
        })
    ...
    private fun PermissionDialog(massage:String){
        ...
        mbsAddAccountDialog.txtUpgrade.setOnClickListener {
            dialog.dismiss()
            val intent = Intent(requireContext(), J4EMembersActivity::class.java)
