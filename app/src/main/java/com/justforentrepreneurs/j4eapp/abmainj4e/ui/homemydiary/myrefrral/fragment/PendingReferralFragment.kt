...
import com.justforentrepreneurs.j4eapp.abmainj4e.ui.homemydiary.myrefrral.adapter.AdapterPendingReferral
...
import com.justforentrepreneurs.j4eapp.databinding.FragmentPendingReferralBinding
import retrofit2.Call
import retrofit2.Callback
import retrofit2.Response


class PendingReferralFragment : Fragment(),AdapterPendingReferral.OnClick2 {
    private lateinit var binding:FragmentPendingReferralBinding
    ...
    override fun onCreateView(
        ...
        // Inflate the layout for this fragment
         binding = DataBindingUtil.inflate(inflater,R.layout.fragment_pending_referral, container, false)
         recyclerData()
         return binding.root
    ...
    private fun getRequirements(){
        ...
        val call = request.app_get_pending_referral(readUsingSharedPreference(requireActivity(), "userid").toString())
                ...
                if (response.isSuccessful) {
                    if (response.body()?.status == true) {
                        if (response.body()?.data != null) {
                            ...
                            val mAdapter = AdapterPendingReferral(
                                activity,
                                response.body()?.data as MutableList<Data>,this@PendingReferralFragment,"given")
    ...
    override fun OnClick(position: Int, data: String) {
        ...
        intent.putExtra("referral_type","given")
        intent.putExtra("back_type","ref_given")
